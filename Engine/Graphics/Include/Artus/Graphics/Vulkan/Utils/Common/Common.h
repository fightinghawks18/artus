...
namespace Artus::Graphics::Vulkan {
    inline vk::Rect2D ToVkRect2D(const Structs::Rectangle& rect) {
        vk::Rect2D rect2D = {};
        rect2D.setOffset({rect.x, rect.y}).setExtent({rect.width, rect.height});
        return rect2D;
    }

    inline vk::Extent2D ToVkExtent2D(const Structs::Extent& extent) { return {extent.width, extent.height}; }

    inline vk::Extent3D ToVkExtent3D(const Structs::Extent3D& extent) {
